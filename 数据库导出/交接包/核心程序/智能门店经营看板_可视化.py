...
def render_time_period_marketing(df: pd.DataFrame, time_dimension: str = '日', selected_period: str = None):
    ...
    if time_col in df.columns:
        ...
        # 🔍 调试信息
        with st.expander("🔍 调试信息（数据聚合结果）", expanded=False):
            st.write(f"时间聚合结果：共{len(time_agg)}个时间点")
            st.dataframe(time_agg[[time_col, '订单数', '订单数_上期值', '订单数_环比率']].tail(10))
        ...
        if len(time_agg) >= 1:
            ...
            if selected_period and not selected_period.startswith("全部"):
                ...
                # 🔍 调试信息
                st.write(f"🔍 选择的周期: {selected_period}")
                st.write(f"🔍 匹配到的索引: {list(latest_idx) if len(latest_idx) > 0 else '无'}")
                ...
                if len(latest_idx) > 0:
                    ...
                    # 🔍 调试信息
                    st.write(f"🔍 当前位置: {current_position}")
                    st.write(f"🔍 上期数据: {'存在' if previous is not None else '不存在（第一条数据）'}")
                    
                    # ✅ 新增：检查是否为第一条数据（无环比）
                    if previous is None and current_position == 0:
                        st.info(f"ℹ️ 选择的{time_dimension}期（{selected_period}）是数据中的第一个时间点，无上一期数据进行环比对比。")
                else:
