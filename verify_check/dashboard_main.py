...
def calculate_order_metrics(df, calc_mode: Optional[str] = None):
    ...
    # 
    # ✅ 修复v2025-11-19: 移除佣金兜底逻辑
    # 原因: 佣金兜底会保留利润额=0但有物流费的订单,导致负利润
    # 示例: 214个订单(服务费=0但佣金>0),利润=-¥519.92
    # 结论: 只看平台服务费>0,不考虑佣金
    if calc_mode in ['service_fee_positive', 'all_with_fallback', 'all_no_fallback']:
        # 统一逻辑: 只使用平台服务费,必须>0
    ...
    # 🔍 [调试] 打印剔除统计
    print(f"\n[调试] 剔除平台服务费=0的订单:")
    print(f"  剔除前: {len(order_agg)}个订单, 利润额总和¥{order_agg['利润额'].sum():,.2f}")
    print(f"  剔除后: {len(filtered)}个订单, 利润额总和¥{filtered['利润额'].sum():,.2f}")
    print(f"  剔除: {len(order_agg) - len(filtered)}个订单, 利润额¥{order_agg['利润额'].sum() - filtered['利润额'].sum():,.2f}")
