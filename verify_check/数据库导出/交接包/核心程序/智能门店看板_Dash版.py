...
def create_scientific_confidence_bar_echarts(scientific_data):
    ...
    if '分类置信度' in df.columns:
        print(f"🔍 [置信度分布图] 置信度范围: {df['分类置信度'].min():.3f} - {df['分类置信度'].max():.3f}")
        print(f"🔍 [置信度分布图] 前5条置信度: {df['分类置信度'].head().tolist()}")
    
    # 统计各置信度级别商品数量
    if '分类置信度' not in df.columns:
        return html.Div("⚠️ 数据缺少置信度字段", className="text-center text-muted p-5")
...
def export_tab1_order_report(n_clicks, store_data, cached_agg, cached_comparison, store_id):
    ...
    # 2. 客单价优化建议（增强版 - 基于商品明细分析）
    if len(aov_export) > 0:
        main_zone = aov_export[aov_export['价格分组'] == '主流区']
        low_zone = aov_export[aov_export['价格分组'] == '低价区']
        high_zone = aov_export[aov_export['价格分组'] == '高价区']
        
        if len(main_zone) > 0:
            main_orders_pct = main_zone['订单占比'].sum()
            
            # 检查是否有明细数据
            detail_insight = ""
            if aov_detail_sheets:
                # 分析低价区高频商品
                low_high_freq = []
                for zone in ['0-10元', '10-20元']:
                    if zone in aov_detail_sheets:
                        zone_data = aov_detail_sheets[zone]
                        if '订单覆盖率' in zone_data.columns:
                            high_freq = zone_data[zone_data['订单覆盖率'] > 50]
                            if len(high_freq) > 0:
                                low_high_freq.extend(high_freq['商品名称'].head(3).tolist())
                
                if low_high_freq:
                    detail_insight = f"，发现高频低价商品：{', '.join(low_high_freq[:3])}"
            
            suggestions.append({
                '类别': '客单价优化',
                '发现': f"主流区（20-50元）订单占比{main_orders_pct:.1f}%{detail_insight}",
                '数据': f"平均客单价¥{avg_order_value:.2f}",
                '建议': "建议：1)低价商品设计组合套餐 2)满减活动引导至50-100元 3)关注3.X系列Sheet查看各区间商品明细"
            })
        
        # 添加低价区和高价区的额外建议
        if len(low_zone) > 0:
            low_orders_pct = low_zone['订单占比'].sum()
            if low_orders_pct > 30:
                suggestions.append({
                    '类别': '低价区分析',
                    '发现': f"低价区（0-20元）订单占比{low_orders_pct:.1f}%过高",
                    '数据': f"建议查看Sheet 3.1和3.2了解具体商品",
                    '建议': "优化策略：设计9.9元、19.9元组合套餐，引导用户凑单提升客单价"
                })
