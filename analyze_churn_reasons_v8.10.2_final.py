...
def analyze_churn_reasons_v8_10_2(
    df: pd.DataFrame,
    ...
    # 5.1 为每个客户添加最后购买日期
    customer_last_dates = churn_customers.set_index('customer_id')['last_order_date'].to_dict()
    top3_per_customer['customer_last_order_date'] = top3_per_customer['customer_id'].map(customer_last_dates)
    
    # 5.2 计算每个商品在不同时期的价格
    # 为了向量化，我们简化为：比较客户历史购买价 vs 近7天平均价
