    ...
    try:
        ...
        for batch_idx, start_idx in enumerate(range(0, len(df), batch_size), 1):
            ...
            try:
                ...
                # 定义冲突时的更新策略
                # 当 order_id 重复时，更新所有字段（保留最新数据）
                # 排除 id, created_at 不更新
                update_dict = {
                    col.name: col 
                    for col in stmt.excluded 
                    if col.name not in ['id', 'created_at', 'product_id', 'profit', 'profit_margin']
                }
                
                upsert_stmt = stmt.on_conflict_do_update(
                    index_elements=['order_id'],  # 冲突检测字段
                    set_=update_dict  # 更新内容
                )
                ...
                # 统计插入和更新数量（PostgreSQL 特性）
                # 注意：SQLAlchemy 可能不直接返回详细统计，这里简化处理
                batch_count = len(orders_data)
                stats['inserted'] += batch_count  # 简化：全部计为处理成功
                
                print(f"[OK] 成功处理 {batch_count} 条")
    ...
    try:
        ...
        if latest_orders:
            ...
            for order in latest_orders:
                print(f"   {order.date} | {order.product_name} | ¥{order.sale_price}")
